...
@section('content')
    ...
    <div>
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <a href="{{ route('district.create') }}" class="btn text-white btn-sm rounded-pill px-3 py-2"
                style="background-color: #2C3E50">Create</a>
        </div>
        <div class="table-responsive">
            <table class="table district-datatable table-hover pt-3 w-100">
                <thead>
                    <tr>
                        <th>S.N:</th>
                        <th>District</th>
...
@push('script')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $('.district-datatable').DataTable({
                processing: true,
                serverSide: true,
                searchDelay: 1000,
            ...
            $(document).on('click', '.toggle-status-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/admin/district/toggle-status/${slug}`,
                            type: 'PATCH',
                            data: {
                                status: newStatus,
                                _token: '{{ csrf_token() }}',
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.status === 'success') {
                                    Swal.fire(
                                        'Deleted!',
                                        'Your district has been deleted.',
                                        'success'
                                    );
                                    table.ajax
