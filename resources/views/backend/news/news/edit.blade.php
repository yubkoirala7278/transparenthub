                ...
                <div class="card-body">
                    <form action="{{ route('news.update', $news->slug) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="description" rows="4" name="description" placeholder="Enter Description">{{ old('description', $news->description) }}</textarea>
                                ...
                                @if (count($categories) > 0)
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('category') == $category->id || $news->news_categories->id == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                @endif
                                ...
                                @if (count($sources) > 0)
                                    @foreach ($sources as $source)
                                        <option value="{{ $source->id }}"
                                            {{ old('source') == $source->id || (isset($news->news_sources) && $news->news_sources->id == $source->id) ? 'selected' : '' }}>
                                            {{ $source->name }}
                                        </option>
                                    @endforeach
                                @endif
                            ...
                            <div class="d-flex align-items-center" style="gap: 20px">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="status" id="active"
                                        value="active" {{ old('status', $news->status) === 'active' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="active">Active</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="status" id="inactive"
                                        value="inactive"
                                        {{ old('status', $news->status) === 'inactive' ? 'checked' : '' }}>
