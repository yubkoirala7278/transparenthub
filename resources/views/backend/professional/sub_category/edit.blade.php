    ...
    <div>
        ...
        <div>
            <div class="card shadow" style="border-top:none;">
                ...
                <div class="card-body">
                    <form action="{{ route('professional_sub_category.update', $sub_category->slug) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                @if ($categories->count() > 0)
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ (old('category_id', $sub_category->professional_categories_id ?? '') == $category->id) ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                @endif
